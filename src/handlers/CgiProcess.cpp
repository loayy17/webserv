...
void CgiProcess::init(pid_t pid, int writeFd, int readFd, const String* clientBuffer, size_t bodyOffset, size_t bodyLength,
                      size_t requestSize) {
    reset();
    _pid          = pid;
    _writeFd      = writeFd;
...
void CgiProcess::appendBuffer(const String& data) {
    if (!_useInternal) {
        if (_clientBuffer) {
            _internalBuffer.append(_clientBuffer->c_str() + _bodyOffset, _bodyLength);
        }
        _useInternal = true;
        _bodyOffset = 0;
    }
