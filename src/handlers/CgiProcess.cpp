...
void CgiProcess::appendBuffer(const char* data, size_t len) {
...
bool CgiProcess::isActive() const { return _active; }
pid_t CgiProcess::getPid() const { return _pid; }
int CgiProcess::getWriteFd() const { return _writeFd; }
int CgiProcess::getReadFd() const { return _readFd; }
time_t CgiProcess::getStartTime() const { return _startTime; }
const String& CgiProcess::getOutput() const { return _output; }

void CgiProcess::setWriteFd(int fd) { _writeFd = fd; }
void CgiProcess::setReadFd(int fd) { _readFd = fd; }

void CgiProcess::closeWriteFd() {
...
bool CgiProcess::writeBody(int fd) {
    ...
    while (_writeOffset < _writeBuffer.size() && totalWritten < BUFFER_SIZE) {
    ...
    if (madeProgress)
        _startTime = getCurrentTime();
    // Compact buffer only if we've written a significant amount or finished
    if (_writeOffset > 0 && (_writeOffset > BUFFER_SIZE || _writeOffset >= _writeBuffer.size())) {
...
bool CgiProcess::handleRead() {
    ...
    while ((n = read(_readFd, buf, sizeof(buf))) > 0) {
        if (_output.size() + n > _output.capacity())
            _output.reserve(_output.size() + n + BUFFER_SIZE);
